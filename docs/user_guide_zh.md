...
GUI 使用单屏左右分栏，不再切换多个标签页：

| 区域 | 内容 |
|------|------|
| 左侧 | INP/DAT/输出目录、预分析按钮、变量预设、Step/NSET/ELSET/Increment 筛选 |
| 右侧 | 预分析结果、输出选项、开始提取、进度条、日志、打开输出目录 |

### 4.3 推荐流程

1. **选择文件**  
   在左侧拖放或选择 `.inp` 和 `.dat` 文件。输出目录可留空，程序会自动生成 `<inp文件名>_adb_output`。

2. **预分析并选择范围**  
   点击左侧“预分析”。右侧会显示模型摘要、Step/Increment、NSET、ELSET 和简单单元连接信息。勾选需要的 Step、节点集、单元集后，点击“应用选中范围”。

3. **选择变量**  
   在左侧使用“节点结果 / 单元结果 / 接触结果 / 弹簧结果”快捷按钮，也可以逐个勾选变量。不勾选任何变量表示提取全部可识别变量。

4. **输出与运行**  
   在右侧选择 CSV/TSV、编码、小数位、是否包含元数据、是否附带节点坐标、是否合并到一个文件。点击“开始提取”后，完成时可直接打开输出目录。
